...
Остальное установка настраивает сама: создает зону файрвола, которая заворачивает трафик локальной сети в туннель, включает автозапуск сервиса, а `luci-app-fptn` перезапускает `rpcd`, чтобы появилась страница в веб-интерфейсе.
