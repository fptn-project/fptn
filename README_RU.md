...
Страница веб-интерфейса — отдельный пакет `luci-app-fptn`, один файл на все архитектуры (`luci-app-fptn-<версия>-openwrt-<ветка>-all.ipk` или `-noarch.apk`). Ставится после клиента:
...
Если FPTN запускает ZeroBlock, страница не нужна: ZeroBlock поднимает клиент со своим конфигом, а вторая страница настроек только путает. При обновлении с 0.4.25 и старше, где страница жила внутри `fptn-client`, сначала обновите клиент и только потом ставьте `luci-app-fptn` — в обратном порядке opkg сочтёт файлы принадлежащими обоим пакетам.

Во время установки роутеру нужен работающий интернет: `kmod-tun` и `ip-full` тянутся из репозитория OpenWrt.

Остальное установка настраивает сама: создает зону файрвола, которая заворачивает трафик локальной сети в туннель, включает автозапуск сервиса, а `luci-app-fptn` перезагружает `rpcd`, чтобы появилась страница в веб-интерфейсе. Именно перезагружает, а не перезапускает: перезапуск `rpcd` оборвал бы сессию LuCI у того, кто ставит пакет.
