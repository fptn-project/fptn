...
| `--preferred-server` | — | Имя сервера, к которому подключаться без гонки. Регистр не важен. При совпадении имён в разных токенах уточняется как `Сервис/Имя`. Можно перечислить несколько имён через запятую (`eu-1, eu-2`): они пробуются по порядку, берётся первый ответивший, а если не ответил никто — идёт гонка по всему пулу |
...
| `--state-file` | путь в `/tmp` по номеру SOCKS-порта | Где помнить текущий сервер и задержки пула, чтобы после рестарта логиниться сразу в рабочий сервер, а не гонять пул заново. `-` — не вести файл |
...
1. если задан `--preferred-server` — его имена пробуются по порядку, берётся
   первый ответивший. Имя, которого нет ни в одном токене, называется в
   предупреждении и пропускается; если не ответил никто — это тоже не фатально,
   выбор переходит к гонке ниже, а процесс не завершается;
2. иначе, если в файле состояния записан сервер и он ещё есть в пуле, сначала
   пробуется он один: он отвечал, когда процесс в прошлый раз останавливался, а
   рестарт обычно означает рестарт демона, а не перемену в мире. Один логин
   вместо гонки;
3. иначе идёт **гонка логинов** (по 16 серверов параллельно, по шесть секунд на
   каждый), выигрывает ответивший первым;
4. при заданном `--max-ping` победителя дополнительно замеряют; не уложился —
   выбывает из списка, и гонка повторяется. Три круга, потом берётся лучший из
   оставшихся.

### Что помнится между запусками

В файле состояния лежит сервер, который был в работе, и последние задержки
пула. Это даёт две вещи.

**Рабочий сервер пробуется первым.** Иначе на публичном пуле из сорока серверов
каждый рестарт — это сорок логинов ради того, что прошлый запуск уже знал.

**Пул гоняется в осмысленном порядке.** Гонка идёт по восемь и останавливается
на первом ответе, так что длительность старта решает состав первой волны.
Впереди — серверы, которые в прошлый раз были быстрыми, за ними неизмеренные,
последними те, что не ответили. Мёртвый узел держит рабочий слот весь таймаут, и
восемь таких в голове списка стоят шести секунд до начала второй волны.

Файл пишется после выбора сервера, после каждого переезда и после каждого
обхода — свежие замеры стоит сохранить независимо от того, привели они к
переезду или нет. Запись атомарная: старт, попавший на середину записи, увидит
либо старое состояние, либо новое, но не половину каждого. Потеря файла не
стоит ничего, кроме гонки: отсутствующий, нечитаемый или битый файл означает
просто старт по-старому.

Путь по умолчанию — в `/tmp` и с номером SOCKS-порта в имени, чтобы демон,
поднимающий по помощнику на секцию, не заставлял их писать друг поверх друга. На
роутере `/tmp` — это RAM: файл переживает перезапуск службы, ради чего он и
нужен, и не переживает ребут. Если указать `--state-file` на флеш, переживёт и
ребут — ценой записи на каждый обход.
...
| `--status-listen-address` | `127.0.0.2` | Адрес, когда через `--status-listen-port` задан только порт. Не действует, если адрес уже назван в `--status-listen` |
| `--status-listen-port` | `0` | Порт API, адрес берётся из `--status-listen-address`. Для управляющей службы, которая раздаёт порты: `0` оставляет API выключенным, если только его не поднял `--status-listen` |
...
С 0.4.26 страница веб-интерфейса — отдельный пакет `luci-app-fptn`
(`luci-app-fptn-<версия>-openwrt-24.10.x-all.ipk`, на 25.12 — `-noarch.apk`).
В `fptn-client` её больше нет, поэтому на роутере, где клиент ставит ZeroBlock,
второй страницы настроек не появляется. Если страницу всё же поставить, а FPTN
запускает секция ZeroBlock, вместо формы она показывает об этом плашку.
