...
void RouteManager::ApplyKillSwitch() {
  ...
  if (tun_interface_name_.empty()) {
    SPDLOG_WARN("Cannot apply Kill Switch: TUN name is empty");
    return;
  }
  SPDLOG_INFO("Applying Kill Switch via {}", tun_interface_name_);
#ifdef __linux__
  fptn::common::system::command::run(
...
#elif __APPLE__
  fptn::common::system::command::run(fmt::format(
      "route -n add -net 0.0.0.0/0 -interface {}", tun_interface_name_));
  fptn::common::system::command::run(fmt::format(
      "route -n add -net ::/0 -interface {}", tun_interface_name_));
#elif _WIN32
